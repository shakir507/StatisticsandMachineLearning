...
# Define a function to perform a single bootstrap iteration
def bootstrap_iteration(i, X_train, Y_train, X_test):
...
# Parallelize the bootstrap predictions
num_cores = 4
n_bootstraps = 100
with concurrent.futures.ProcessPoolExecutor(max_workers=num_cores) as executor:
    results = executor.map(bootstrap_iteration, range(n_bootstraps), [X_train]*n_bootstraps, [Y_train]*n_bootstraps, [X_test]*n_bootstraps)
    bootstrap_predictions = list(results)
